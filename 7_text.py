...
# 적 캐릭터
enemy = pygame.image.load('C:/Users/PC1/Desktop/PYTHONWORKSPACE/pygame_basic/enemy.png')
enemy_size = enemy.get_rect().size # 사각형 크기를 반환.
enemy_width = enemy_size[0] # 캐릭터의 가로 크기.
enemy_height = enemy_size[1] # 캐릭터의 세로 크기.
enemy_x_pos = (screen_width / 2) - (enemy_width / 2) # 화면 가로의 가운데에 해당하는 곳에 위치하도록.
enemy_y_pos = (screen_height / 2) - (enemy_height / 2)  # 화면 세로의 가장 아래에 해당하는 곳에 위치.

# 폰트 정의
game_font = pygame.font.Font(None, 40) # 폰트는 디폴트, 크기는 40 명시.

# 총 시간
total_time = 10 # 10초 안에 게임해야한다고 가정.
...
while running:
    ...
    character_rect.left = character_x_pos # 이동한 위치를 업데이트
    ...
    # 충돌 체크
    if character_rect.colliderect(enemy_rect): # enemy와 충돌했는지를 확인
        print('충돌')
    ...
    screen.blit(enemy, (enemy_x_pos, enemy_y_pos)) # 적군 캐릭터 위치에 따라서 입력.


    # 타이머 넣기.
    # 경과 시간 계산.
    elaped_time = (pygame.time.get_ticks() - start_ticks) / 1000 
    # 단위가 ms이기 때문에 1000으로 나누어 초단위로 표시.

    # render <- 글적기.
    timer = game_font.render(str(int(total_time - elaped_time)), True, (255, 255, 255)) # int 단위로 남은 게임 시간을 적음.
    # render 안에는 str, True, 글자 색상.
    screen.blit(timer, (10,10)) # 10, 10 위치에 글자 표시.
...
pygame.time.delay(1000) # 1초 정도 끝나기 전에 대기.
